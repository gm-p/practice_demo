...
namespace sola
{
    ...
    static const char normal[] = {0x1b, '[', '1', ';', '3', '9', 'm', 0};
    ...
    void logger::error(const std::string& msg, const std::string& file, std::size_t line)
    {
        ...
        {
            std::lock_guard<std::mutex> lock(m_mutex);
            std::cout<<"["<<red<<"ERROR"<<normal<<"][sola::logger]["<<file<<":"<<line<<"] "<<msg<<std::endl;
        }
    }
    ...
    thread_pool::thread_pool(int init_size): m_init_threads_size(init_size), m_mutex(), m_cond(), m_is_started(false)
    {
        start();
    }
