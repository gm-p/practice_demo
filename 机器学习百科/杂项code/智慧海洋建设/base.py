...
def read_feat(path, test_mode=False):
    ...
    df_feat.append(dt['time'].dt.hour.value_counts().index[0])
...
test_feat = Parallel(n_jobs=10)(delayed(read_feat(path, False)
                                        for path in glob.glob('../input/hyround1_testA_20200102/*')[:]))
...
eval_fun = f1_score
def fun_oof(clf, X_train, y_train, X_test, kf):
    print(clf)
...
'''
https://tianchi.aliyun.com/notebook-ai/detail?spm=5176.12586969.1002.3.163c5cfdJTbd9E&postId=87376
'''