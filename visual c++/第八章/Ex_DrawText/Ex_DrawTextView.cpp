...
void CEx_DrawTextView::OnDraw(CDC* pDC)
{
	...
	pDC->Rectangle(rc);
	int nTab=40;
	pDC->TabbedTextOut(rc.left,rc.top,"绘制\tTab\t文本\t示例",1,&nTab,rc.left);
	nTab=80;
	pDC->TabbedTextOut(rc.left,rc.top+20,"绘制\tTab\t文本\t示例",1,&nTab,rc.left);
	pDC->TabbedTextOut(rc.left,rc.top+40,"绘制\tTab\t文本\t示例",0,NULL,0);
}
