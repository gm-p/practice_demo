...
struct bigInteger  //高精度整数结构体
{
	...
	void set(char str[])//从字符串中提取整数
	{
		...
		for(int i=L-1,j=0,t=0,c=1;i>=0;i--) //从最后一个字符开始倒序遍历字符串，j控制每4个
											//字符转换为一个数字存入数组，t临时保存字符转换为数字的中间
											//值，c表示当前位的权重，按1，10，100，1000顺序变化
		 {
		 	t+=(str[i]-'0')*c;//计算这个四位数 中当前字符代表的数字，即数字乘以当前位权重
			j++;//当前转换字符数增加
			c*=10;//计算下一位权重
